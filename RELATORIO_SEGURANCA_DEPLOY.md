...
- **Credenciais gerenciadas pela aplicação:** guardar com **criptografia autenticada em repouso**, por exemplo AES-256-GCM ou ChaCha20-Poly1305. Cada registro deve conter `ciphertext`, `nonce` único e identificador de versão/chave; a autenticação deve cobrir também o contexto do registro (por exemplo, id da conta e nome do campo) como AAD. Criptografar `accounts.password` e todos os valores secretos de `field_values`; revisar quais campos livres devem ser classificados como segredo.
...
1. Não selecionar, renderizar, serializar em JavaScript ou colocar em atributos HTML nenhum segredo por padrão — nem `accounts.password`, nem valores de `field_values` classificados como secretos. A listagem deve devolver apenas metadados não secretos; incluir flag/classificação de segredo no esquema para distinguir campos que podem ser exibidos de campos que exigem reveal.
2. Criar endpoint `POST` autenticado e autorizado para revelar/copiar **um** segredo por vez — senha ou campo secreto. Exigir reautenticação recente para essa ação (senha atual nos últimos 5 minutos), validar a permissão no servidor e retornar somente o valor solicitado.
3. Registrar em auditoria quem revelou qual registro/campo e quando, sem registrar o valor; aplicar rate limit e expiração curta ao resultado. Não persistir o valor em `localStorage`, logs ou histórico.
4. Para edição, pedir explicitamente um novo valor secreto; não pré-preencher senha ou campo secreto no modal. Alterar somente os campos enviados, sem reenviar segredo ao mudar status.
